...
"""     def test__list_view(self):
    ...
    def test_room_detail_view(self):
        response = self.client.get(reverse('room_create', kwargs={ 'pk': self.building.id ,'room_pk' : self.room.id }))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'room/detail.html')
        self.assertContains(response, self.building.creator)
    
    def test_room_detail_view_logout_auth_user(self):
        logout_client = Client()
        response = logout_client.get(reverse('building_detail', kwargs={ 'pk': self.building.id ,'room_pk' : self.room.id }))
        self.assertEqual(response.status_code, 302)
    
    def test_room_create_view(self):
        response = self.client.post(reverse('room_create'),{ 'area' : '12' ,'capacity' : '53','room_name' : 'testroom',}, kwargs={ 'pk': self.building.id })
        self.assertEqual(response.status_code, 302)
        self.assertTrue(Building.objects.filter(area = '12').exists())
     """